...
# Motor de cálculo técnico estable y fijo para el contrato
def get_sniper_signal():
    ...
    if not df.empty and len(df) > 14:
        ...
        score = 60 if change_pct >= 0 else 40
...
# Estilos visuales exactos que te gustaban
st.markdown("""
<style>
...
# Interfaz visual recuperada
st.markdown('<div class="badge-alert">🎯 BTC CONTRACT SNIPER - SEÑAL FIJA</div>', unsafe_allow_html=True)
...
<div class="text-sub">Variación inmediata: {change_pct:+.2f}%</div>
</div>""", unsafe_allow_html=True)

# Probabilidad Técnica Principal con Barra
...
# Desglose Fuerza de Mercado en Cajas
...
st.markdown('<div class="footer-credits">Alpha Bot Sniper | Fixed Contract Engine</div>', unsafe_allow_html=True)
